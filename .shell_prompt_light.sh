...
function __promptline_battery {
  ...
  for possible_battery_dir in /sys/class/power_supply/BAT*; do
    if [[ -d $possible_battery_dir && -f "$possible_battery_dir/charge_full_design" && -f "$possible_battery_dir/charge_now" ]]; then
      current_capacity=$( <"$possible_battery_dir/charge_now" )
      battery_capacity=$( <"$possible_battery_dir/charge_full_design" )
      local battery_level=$(($current_capacity * 100 / $battery_capacity))
      [[ $battery_level -gt $threshold ]] && return 1
...
function __promptline {
  ...
  local x_bg="${wrap}48;5;7${end_wrap}"
  local x_sep_fg="${wrap}38;5;7${end_wrap}"
